define(["require", "exports", "tslib", "chai", "Types/_source/HierarchicalMemory", "Types/_source/Memory", "Types/_source/Query", "Types/_entity/Model", "Types/_entity/Record"], function (require, exports, tslib_1, chai_1, HierarchicalMemory_1, Memory_1, Query_1, Model_1, Record_1) {
    "use strict";
    ...
    describe('Types/_source/HierarchicalMemory', function () {
        ...
        beforeEach(function () {
            ...
            source = new HierarchicalMemory_1.default({
                data: data,
                keyProperty: 'id'
            });
        });
        ...
        describe('.getOriginal()', function () {
            it('should return Memory instance', function () {
                chai_1.assert.instanceOf(source.getOriginal(), Memory_1.default);
            });
        });
        ...
        describe('.read()', function () {
            it('should return record', function () {
                return source.read(1).then(function (model) {
                    chai_1.assert.instanceOf(model, Model_1.default);
                    chai_1.assert.equal(model.getId(), 1);
                });
            });
        });
        describe('.update()', function () {
            it('should update record', function () {
                var rec = new Record_1.default({
        ...
        describe('.query()', function () {
            it('should return all source records', function () {
                return source.query().then(function (result) {
                    chai_1.assert.equal(result.getAll().getCount(), 2);
                });
            });
            it('should return items and path in metadata', function () {
                var source = new HierarchicalMemory_1.default({
                    data: [
                ...
                return source.query(query).then(function (result) {
                    ...
                    chai_1.assert.deepEqual(items, expectItems);
                    var path = [];
                    result.getAll().getMetaData().path.each(function (item) {
                        path.push(item.get('name'));
                    });
                    chai_1.assert.deepEqual(path, expectPath);
                });
            });
        });
        ...
        describe('.copy()', function () {
            it('should copy record', function () {
                return source.copy(1).then(function (result) {
                    chai_1.assert.equal(result.getId(), 1);
                });
            });
        });
        ...
        describe('.toJSON()', function () {
            it('should serialize its own options', function () {
                ...
                var source = new HierarchicalMemory_1.default(tslib_1.__assign({ data: data }, options));
                var serialized = source.toJSON();
                chai_1.assert.deepEqual(serialized.state.$options, options);
                chai_1.assert.deepEqual(serialized.state._source._$data, data);
            });
        });
    });
});
